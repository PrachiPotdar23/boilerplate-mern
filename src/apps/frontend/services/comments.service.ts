...
export default class CommentsService extends APIService {
  addComment = async (taskId: string, comment: string): Promise<ApiResponse<Comment>> => {
    try {
      const userAccessToken = JSON.parse(localStorage.getItem('access-token')) as AccessToken;
      const response = await this.apiClient.post(`/tasks/${taskId}/comments`, { comment }, {
        headers: {
  ...
  getComments = async (taskId: string): Promise<ApiResponse<Comment[]>> => {
    try {
      ...
      const comments: Comment[] = (response.data as JsonObject[]).map((commentData) => new Comment(commentData));
